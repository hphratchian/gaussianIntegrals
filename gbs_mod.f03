...
!
      ...
      real(kind=real64),dimension(:),allocatable::primitiveExponents,  &
        contractionCoefficients,coordinates
      type(MQC_Variable)::tmp
!
 1000 format(/,1x,'Data in loadGaussianBasisSet',/,  &
        3x,'nBasis=',i6,3x,'nShells=',i6,3x,'nPrims=',i6,/)
!
!     Get the key parameters of the basis set on the FAF and initialize the
!     basisSet object.
!
      ...
      call basisSet%init(nShells)
!
!     Read in the shell info from faf so we can load basisSet.
!
!     NOTE: The FAF entry "P(S=P) CONTRACTION COEFFICIENTS" is NOT currently
!           being loaded. We might need to fix this later.
!
